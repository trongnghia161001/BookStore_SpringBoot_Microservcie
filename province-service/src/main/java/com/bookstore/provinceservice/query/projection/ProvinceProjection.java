...
public class ProvinceProjection {
    ...
    @QueryHandler
    public List<ProvinceResponseCommonModel> handle(GetProvinceByNameQuery getProvinceByNameQuery) {
        List<ProvinceResponseCommonModel> list = new ArrayList<>();
        List<Province> List = repository.findByNameContainingIgnoreCase(getProvinceByNameQuery.getProvinceName());
        List.forEach(book -> {
            ProvinceResponseCommonModel model = new ProvinceResponseCommonModel();
            BeanUtils.copyProperties(book, model);
            list.add(model);
